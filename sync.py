...
try:
    ...
    KALENDARZ_DB_ID = os.environ["KALENDARZ_DB_ID"]  # ID BAZA "KALENDARZ PRACY"
    AKTYWNOSCI_DB_ID = os.environ["AKTYWNOSCI_DB_ID"]  # ID BAZA "AKTYWNOŚĆ – LOG"
    WYNIKI_DB_ID = os.environ["WYNIKI_DB_ID"]  # ID BAZA "Wyniki KPI"
except KeyError as e:
    print(f"FATAL BŁĄD brak Secretów: {e}")
    exit(1)
...
def generate_kpi_bar(current_val, target_val):
    ...
    if percent >= 100: icon = "✅"
    elif percent >= 70: icon = "⚠️"
    else: icon = "🚨"
    
    if current <= target:
        filled_count = round((current / target) * blocks_base)
        char = "🟩" if percent >= 100 else ("🟨" if percent >= 70 else "🟥")
...
def find_or_create_record(name, db_id, typ, date_ref, period_text):
    ...
    page = notion.pages.create(parent={"database_id": db_id}, properties={
        ...
        "Typ": {"select": {"name": typ}},  # AUTO: Dzień / Tydzień / Miesiąc
        "Data": {"date": {"start": str(date_ref)}},
        "Okres": {"rich_text": [{"text": {"content": period_text}}]}
    })
    print(f"➕ Utworzono nowy wpis: {name} ({typ})")
    return page["id"], True

def update_kpi(page_id, op_str, kon_str, sprz_str):
    notion.pages.update(page_id=page_id, properties={
        "Operacje KPI": {"rich_text": [{"text": {"content": op_str}}]},
        "Kontakt KPI": {"rich_text": [{"text": {"content": kon_str}}]},
        "Sprzedaż KPI": {"rich_text": [{"text": {"content": sprz_str}}]}
    })
...
def aggregate_period(start_date, end_date):
    ...
    for row in res:
        p = row["properties"]
        plan_op = p.get("Target operacje", {}).get("number", 0) or 0
        plan_kon = p.get("Target kontakty", {}).get("number", 0) or 0
        plan_sprz = p.get("Target sprzedaż", {}).get("number", 0) or 0
        
        real_op = p.get("Operacje (real)", {}).get("number", 0) or 0
        real_kon = p.get("Kontakty (real)", {}).get("number", 0) or 0
        real_sprz = p.get("Sprzedaż (real)", {}).get("number", 0) or 0
        
        totals["plan_op"] += plan_op
        totals["plan_kon"] += plan_kon
        totals["plan_sprz"] += plan_sprz
        totals["op"] += real_op
        totals["kon"] += real_kon
        totals["sprz"] += real_sprz
...
if not dzien_robo:
    print("⏸️ BRAK DNIA ROBOCZEGO! Wyjście.")
    exit(0)
...
plan_op = pobierz_plany.get("Target operacje", {}).get("number") or 0
plan_kon = pobierz_plany.get("Target kontakty", {}).get("number") or 0
plan_sprz = pobierz_plany.get("Target sprzedaż", {}).get("number") or 0

# UŻYWAMY Pola `Data` (nie `Data (dzień)` - to relacja)
...
for akt in act_list:
    ...
    if props.get("Operacje flag"): akt_op += props["Operacje flag"].get("number") or 0
    if props.get("Kontakt flag"): akt_kon += props["Kontakt flag"].get("number") or 0
    if props.get("Sprzedaż flag"): akt_sprz += props["Sprzedaż flag"].get("number") or 0
...
notion.pages.update(page_id=dzien_id, properties={
    "Operacje (real)": {"number": int(akt_op)}, 
    "Kontakty (real)": {"number": int(akt_kon)}, 
    "Sprzedaż (real)": {"number": int(akt_sprz)}
})
...
update_kpi(id_dnia, generate_kpi_bar(akt_op, plan_op), generate_kpi_bar(akt_kon, plan_kon), generate_kpi_bar(akt_sprz, plan_sprz))
...
update_kpi(id_tydz, generate_kpi_bar(tot_tydz["op"], tot_tydz["plan_op"]), generate_kpi_bar(tot_tydz["kon"], tot_tydz["plan_kon"]), generate_kpi_bar(tot_tydz["sprz"], tot_tydz["plan_sprz"]))
...
update_kpi(id_mies, generate_kpi_bar(tot_mies["op"], tot_mies["plan_op"]), generate_kpi_bar(tot_mies["kon"], tot_mies["plan_kon"]), generate_kpi_bar(tot_mies["sprz"], tot_mies["plan_sprz"]))

print("\n🎉 DONE: Wszystko zsynchronizowane!")
